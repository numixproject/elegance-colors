...
class EleganceColorsWindow : ApplicationWindow {
	...
	void export_theme () {
		...
		if (apply_button.get_sensitive ()) {
			var dialog = new Gtk.Dialog.with_buttons ("Apply changes?", this,
									Gtk.DialogFlags.MODAL,
									Gtk.Stock.APPLY, Gtk.ResponseType.APPLY,
									Gtk.Stock.CANCEL, Gtk.ResponseType.CANCEL, null);

			var content_area = dialog.get_content_area ();
			var label = new Gtk.Label ("You need to apply the changes first to export the theme!");
			...
			var namedialog = new Gtk.Dialog.with_buttons ("Export theme", this,
									Gtk.DialogFlags.MODAL,
									Gtk.Stock.OK, Gtk.ResponseType.OK,
									Gtk.Stock.CANCEL, Gtk.ResponseType.CANCEL, null);

			var name_label = new Gtk.Label ("Please enter a name for the theme");
			var name_area = namedialog.get_content_area ();
			name_box = new Gtk.Entry ();

			name_area.border_width = 5;
			name_area.spacing = 10;
			name_area.add (name_label);
			name_area.add (name_box);

			namedialog.response.connect (on_export);

			namedialog.show_all ();
		}
	}

	void on_export (Gtk.Dialog namedialog, int response_id) {
		...
		case Gtk.ResponseType.OK:
			...
			try {
				Process.spawn_command_line_sync("elegance-colors export \"%s\"".printf (name));
			} catch (Error e) {
				stderr.printf ("Could not export theme: %s\n", e.message);
			}

			if (File.new_for_path (Environment.get_home_dir ()).get_child ("%s.zip".printf(name)).query_exists ()) {
				var dialog = new Gtk.Dialog.with_buttons ("Saved!", this,
										Gtk.DialogFlags.MODAL,
										Gtk.Stock.OK, Gtk.ResponseType.OK, null);

				var label = new Gtk.Label ("Theme exported to %s/%s.zip.".printf (Environment.get_home_dir (), name));
				var content_area = dialog.get_content_area ();
				...
				var dialog = new Gtk.Dialog.with_buttons ("Failed!", this,
										Gtk.DialogFlags.MODAL,
										Gtk.Stock.OK, Gtk.ResponseType.OK, null);

				var label = new Gtk.Label ("Failed to export the theme.");
				var content_area = dialog.get_content_area ();
	...
	void create_widgets () {
		...
		font_size.set_halign (Gtk.Align.END);
		...
		dashgradient_label.set_halign (Gtk.Align.START);
		dashgradient_size = new SpinButton.with_range (0, 255, 1);
		dashgradient_size.set_halign (Gtk.Align.END);
		...
		panel_bordercol_label.set_halign (Gtk.Align.START);
		panel_bordercol_color = new ColorButton ();
		panel_shadow_label = new Label.with_mnemonic ("Drop shadow");
		panel_shadow_label.set_halign (Gtk.Align.START);
		panel_shadow_switch = new Switch ();
		panel_shadow_switch.set_halign (Gtk.Align.END);
		...
		menu_bordercol_label.set_halign (Gtk.Align.START);
		menu_bordercol_color = new ColorButton ();
		menu_shadow_label = new Label.with_mnemonic ("Drop shadow");
		menu_shadow_label.set_halign (Gtk.Align.START);
		menu_shadow_switch = new Switch ();
		menu_shadow_switch.set_halign (Gtk.Align.END);
		...
		grid0.attach_next_to (combobox, presets_label, Gtk.PositionType.RIGHT, 2, 1);
		grid0.attach (mode_label, 0, 1, 1, 1);
		grid0.attach_next_to (match_wallpaper, mode_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach_next_to (match_theme, match_wallpaper, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach_next_to (custom_color, match_wallpaper, Gtk.PositionType.BOTTOM, 1, 1);
		grid0.attach_next_to (color_button, custom_color, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (monitor_label, 0, 4, 2, 1);
		grid0.attach_next_to (monitor_switch, monitor_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (newbutton_label, 0, 5, 2, 1);
		grid0.attach_next_to (newbutton_switch, newbutton_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (entry_label, 0, 6, 2, 1);
		grid0.attach_next_to (entry_switch, entry_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (font_label, 0, 7, 2, 1);
		grid0.attach_next_to (font_size, font_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (selgradient_label, 0, 8, 2, 1);
		grid0.attach_next_to (selgradient_size, selgradient_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (dashgradient_label, 0, 9, 2, 1);
		grid0.attach_next_to (dashgradient_size, dashgradient_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (roundness_label, 0, 10, 2, 1);
		grid0.attach_next_to (corner_roundness, roundness_label, Gtk.PositionType.RIGHT, 1, 1);
		grid0.attach (transition_label, 0, 11, 2, 1);
		grid0.attach_next_to (transition_duration, transition_label, Gtk.PositionType.RIGHT, 1, 1);
		...
		grid1.attach_next_to (panel_bg_color, panel_bg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_fg_label, 0, 1, 2, 1);
		grid1.attach_next_to (panel_fg_color, panel_fg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_bordercol_label, 0, 2, 2, 1);
		grid1.attach_next_to (panel_bordercol_color, panel_bordercol_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_shadow_label, 0, 3, 2, 1);
		grid1.attach_next_to (panel_shadow_switch, panel_shadow_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_icon_label, 0, 4, 2, 1);
		grid1.attach_next_to (panel_icon_switch, panel_icon_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_gradient_label, 0, 5, 2, 1);
		grid1.attach_next_to (panel_gradient_value, panel_gradient_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_opacity_label, 0, 6, 2, 1);
		grid1.attach_next_to (panel_opacity_value, panel_opacity_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_borderop_label, 0, 7, 2, 1);
		grid1.attach_next_to (panel_borderop_value, panel_borderop_label, Gtk.PositionType.RIGHT, 1, 1);
		grid1.attach (panel_corner_label, 0, 8, 2, 1);
		grid1.attach_next_to (panel_corner_value, panel_corner_label, Gtk.PositionType.RIGHT, 1, 1);
		...
		grid2.attach_next_to (menu_bg_color, menu_bg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_fg_label, 0, 1, 2, 1);
		grid2.attach_next_to (menu_fg_color, menu_fg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_bordercol_label, 0, 2, 2, 1);
		grid2.attach_next_to (menu_bordercol_color, menu_bordercol_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_shadow_label, 0, 3, 2, 1);
		grid2.attach_next_to (menu_shadow_switch, menu_shadow_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_arrow_label, 0, 4, 2, 1);
		grid2.attach_next_to (menu_arrow_switch, menu_arrow_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_gradient_label, 0, 5, 2, 1);
		grid2.attach_next_to (menu_gradient_value, menu_gradient_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_opacity_label, 0, 6, 2, 1);
		grid2.attach_next_to (menu_opacity_value, menu_opacity_label, Gtk.PositionType.RIGHT, 1, 1);
		grid2.attach (menu_borderop_label, 0, 7, 2, 1);
		grid2.attach_next_to (menu_borderop_value, menu_borderop_label, Gtk.PositionType.RIGHT, 1, 1);
		...
		grid3.attach_next_to (dialog_bg_color, dialog_bg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_fg_label, 0, 1, 2, 1);
		grid3.attach_next_to (dialog_fg_color, dialog_fg_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_heading_label, 0, 2, 2, 1);
		grid3.attach_next_to (dialog_heading_color, dialog_heading_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_bordercol_label, 0, 3, 2, 1);
		grid3.attach_next_to (dialog_bordercol_color, dialog_bordercol_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_shadow_label, 0, 4, 2, 1);
		grid3.attach_next_to (dialog_shadow_switch, dialog_shadow_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_gradient_label, 0, 5, 2, 1);
		grid3.attach_next_to (dialog_gradient_value, dialog_gradient_label, Gtk.PositionType.RIGHT, 1, 1);
		grid3.attach (dialog_opacity_label, 0, 6, 2, 1);
		grid3.attach_next_to (dialog_opacity_value, dialog_opacity_label, Gtk.PositionType.RIGHT, 1, 1);
		...
		// Tabs
		var tabs = new Box (Gtk.Orientation.HORIZONTAL, 0);
